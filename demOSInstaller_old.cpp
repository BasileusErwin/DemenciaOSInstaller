...
void InstallProcess()
{
	...
	// /usr/lib/live/mount/medium/live/filesystem.squashfs" FOR NEW DEBIAN BASE
	...
	if(isEFI==false)
	{
		...
		// Cambiar a la instalación de destino y ejecutar update-grub para generar la configuración del GRUB
        	cout << "Use genfstab -U /media/target/ >> /media/target/etc/fstab in the other termianl next command\n update-initramfs -u and finally in the same window of the installer write update-grub\n" << endl;
